...
OldParser::OldParser(vector<vector<Token>> inputFromLexer)
{
    // breaks lexer input back into single tokens
    for (auto line : inputFromLexer)
    {
        for (Token t : line)
        {
    ...
    // calculate new ends of expressions for edge cases
...
OldParser::Node *OldParser::constructAST(vector<Token> tokens)
{
    Node *root = nullptr;
    ...
    {
        root = new Node{OldParser::Node{tokens[0], vector<Node *>(), nullptr}};
        return (root);
    }
    ...
    root = new Node{OldParser::Node{tokens[1], vector<Node *>(), nullptr}};
    ...
    for (unsigned int tIndex = 2; tIndex < tokens.size(); tIndex++)
    {
        ...
        if (currentToken.text == "(")
        {
            ...
            Node *operatorNode = new Node{OldParser::Node{tokens[tIndex], vector<Node *>(), currentPtr}};
        ...
        {
            currentPtr->branches.push_back(new Node{OldParser::Node{currentToken, vector<Node *>(), currentPtr}});
        }
...
string OldParser::printHelper(OldParser::Node *top, bool lastChild)
{
    string finalText = "";
    if (!top)
        return finalText;
    bool last;
    // correctly parenthesize final output
...
double OldParser::evaluate(Node *top)
{
    if (!top)
    {
        return 0;
...
void OldParser::parseError(Token token)
{
...
Token OldParser::findParenthesisBefore(Token o)
{
...
vector<Token> OldParser::oneExpression()
{
    int parentheses = 0;
    vector<Token> result;
    while (singleTokens.size() > 0)
